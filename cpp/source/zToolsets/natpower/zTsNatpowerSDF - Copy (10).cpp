...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_SliceMesh(zObjMesh& o_Mesh, int startVID, int endVID, zIntArray& FeaturedNumStrides, bool left)
	{
		...
		for (int i = 0; i < hesFeatureInner.size(); i++)
		{
			...
			{
				innerColor = _colorCornersOuter;
				outerColor = _colorCornersOuter;
			}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_SliceMesh_Regular(zObjMesh& o_Mesh, int startVID, int endVID, zIntArray& FeaturedNumStrides)
	{
		...
		*/
		...
		for (int m = 0; m < hesFeatureInner.size(); m++)
		{
			...
			if (i == 0)
			{
				innerColor = _colorCornersInner;
				outerColor = _colorCornersInner;
			}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_SliceMesh_Top(zObjMesh& o_Mesh, int startVID, int endVID, zIntArray& FeaturedNumStrides)
	{
		...
		vCornerStart.setColor(_colorCornersStart);
		...
		for (int k = 0; k < featureWalkNum * 2; k++)
		{
			...
			innerColor = isCornerEdge ? _colorCornersInner : _colorFeatureInner;
			outerColor = isCornerEdge ? _colorCornersOuter : _colorFeatureOuter;
		...
		if (blockType == zBlockType::Top)
		{
			...
			{
				heWalk.getEdge().setColor(_colorCornersInner);
				heWalk.getVertex().setColor(_colorCornersInner);

				zItMeshHalfEdge heOuter = heWalk.getSym().getNext().getNext();
				if (flip) heOuter = heWalk.getNext().getNext();

				heOuter.getEdge().setColor(_colorCornersOuter);
				heOuter.getVertex().setColor(_colorCornersOuter);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_PrintBlocks(zDomainFloat& _printHeightDomain, float printLayerWidth, float raftLayerWidth, bool allSDFLayers, int& numSDFlayers, int funcNum, int numSmooth, zDomainFloat _neopreneOffset, bool compFrames, bool compSDF)
	{
		...
		o_trimGraphs_cables.clear();
		o_trimGraphs_cables.assign(o_sectionGraphs.size(), zObjGraph());
		...
		o_trimGraphs_interiorSplit.clear();
		o_trimGraphs_interiorSplit.assign(o_sectionGraphs.size(), zObjGraph());
		...
		for (int i = 1; i < o_sectionGraphs.size(); i++)
		{
			...
			{
				compute_TrimGraphs_CableBracing(i, o_trimGraphs_cables[i]);
				fng = zFnGraph(o_trimGraphs_cables[i]);
				fng.setEdgeColor(zBLUE);
				allTrimGraphs.push_back(o_trimGraphs_cables[i]);
			}
			...
			//compute_TrimGraphsHEs_Inner_Arch(i, hes);
	...
	ZSPACE_TOOLSETS_INLINE bool zTsNatpowerSDF::check_sectionGraphGeomCheck(zObjGraph& graph)
	{
		...
		for (zItGraphVertex v(graph); !v.end(); v++)
		{
			...
			if (v.getColor() == _colorCornersStart) corner0 = true;
			if (v.getColor() == _colorCornersEnd) corner1 = true;
			if (v.getColor() == _colorCornersInner) corner2 = true;
			if (v.getColor() == _colorCornersOuter) corner3 = true;
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::slotGraph_Arch(int graphId, float graphLength, zObjGraph& outGraph)
	{
		...
		bool found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersStart, _colorCornersEnd, heEdge);
		if (!found)
		{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_TrimGraphs_Boundary_Arch(int graphId, zObjGraph& outGraph)
	{
		...
		bool found;
		found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersStart, _colorCornersEnd, heEdge1);
		if (!found) printf("\n trimGraph_edg1 was not found - graphID %i", graphId);

		zItGraphHalfEdgeArray heEdge2;
		found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersInner, _colorCornersOuter, heEdge2);
		if (!found) printf("\n trimGraph_edg1 was not found - graphID %i", graphId);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_TrimGraphs_CableBracing(int graphId, zObjGraph& outGraph)
	{
		...
		//00
		found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersStart, _colorCornersEnd, hes0);
		if (found)
		{
			zPoint p;
			float d;
		...
		//01 
		found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersStart, _colorCornersInner, hes1);
		if (found)
		{
			zPoint p;
			float d;
		...
		//02
		found = getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersEnd, _colorFeatureOuter, hes2);
		if (found)
		{
			zPoint p;
			float d;
		...
		if (isCorner)
		{
			...
			for (zItGraphVertex v(o_sectionGraphs[graphId]); !v.end(); v++)
			{
				...
				if (v.getColor() == _colorCornersInner)
				{
					bracingStart = v;
					found == true;
				}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_FrontBackHE(zObjGraph& graph, zItGraphHalfEdgeArray& outHeInner, zItGraphHalfEdgeArray& outHEOuter)
	{
		...
		for (zItGraphVertex v(graph); !v.end(); v++)
		{
			if (v.getColor() == _colorCornersOuter)
			{
				zItGraphHalfEdgeArray hc;
				v.getConnectedHalfEdges(hc);
				for (int i; i < 2; i++)
				{
					if (hc[i].getVertex().getColor() == _colorCornersOuter) he1 = hc[i].getNext();
					else he2 = hc[i];
				}
				break;
			}
		...
		while (safetyCounter < 1000)
		{
			...
			if (he1.getStartVertex().getColor() == _colorCornersOuter) break;
		}
		safetyCounter = 0;
		while (safetyCounter < 1000)
		{
			...
			hes2.push_back(he2);
			he2 = he2.getNext();
			if (he2.getStartVertex().getColor() == _colorCornersOuter) break;
		}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_BlockSDF_Planar_wall(int funcNum, int numSmooth, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		...
		if (false)
		{
			...
			//trim graph
			//compute_PrintBlock_TrimGraphs(o_sectionGraphs[graphId], o_trimGraphs[graphId], topHE, bottomHE);
		}
		...
		zObjGraph slotGraph1;
		float slotRadius = 0.02;
		zObjGraph slotGraph;
		if (funcNum >= 2) fnField.getScalarsAsEdgeDistance(slotField, slotGraph1, 0.20 * pWidth, false);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_BlockSDF_Planar_bracing(int funcNum, int numSmooth, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		...
		fng = zFnGraph(o_trimGraphs_cables[graphId]);
		fng.setTransform(t, true, false);
		fng.setTransform(tLocal, true, true);
		...
		fng.setTransform(t, true, false);
		fng.setTransform(tLocal, true, true);
		fng = zFnGraph(o_trimGraphs_interiorSplit[graphId]);
		fng.setTransform(t, true, false);
		fng.setTransform(tLocal, true, true);
		...
		zItGraphHalfEdgeArray hesExterior;
		///getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersStart, _colorCornersInner, hesInterior);
		getShortestHEsBetweenColors(o_sectionGraphs[graphId], _colorCornersEnd, _colorCornersOuter, hesExterior);
		...
		float bracingEdgeWidth = 0.25 * pWidth;
		...
		outerOffsetArray.assign(fnGraph.numEdges(), offset_outer_interior);
		innerOffsetArray.assign(fnGraph.numEdges(), offset_inner_interior);

		for (zItGraphHalfEdge he : hesExterior)
		{
			outerOffsetArray[he.getEdge().getId()] = offset_outer_exterior;
			innerOffsetArray[he.getEdge().getId()] = offset_inner_exterior;
		}
		...
		splitGraph_1(planeXY, o_sectionGraphs[graphId], offset_inner_exterior*2, pWidth*2, splitGraph);
		o_trimGraphs_interiorSplit[graphId] = splitGraph;
		...
		if (funcNum >= 3)
		{
			...
			zItGraphVertex vCenter(o_trimGraphs_cables[graphId], 0);
			zPoint cableCenter = vCenter.getPosition();

			fnField.getScalars_Circle(scalar_cable, cableCenter, cableRadius, 0, false);

			fnField.getScalarsAsEdgeDistance(scalar_cableBracing, o_trimGraphs_cables[graphId], bracingEdgeWidth, false);
			...
			for (zItGraphEdge e(o_trimGraphs_cables[graphId]); !e.end(); e++)
			{
			...
			for (zItGraphHalfEdge& he : hesTemp)
			{
				...
				float slotOffset = cableRadius + pWidth * 1.5;// : (he.getLength() / 2.0) + (bracingEdgeWidth * 4);
				//if (graphId % 2 == 0) slotOffset += (bracingEdgeWidth * 2);
				if (graphId % 2 == 0) slotOffset += pWidth;
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_BlockSDF_NonPlanar(int funcNum, int numSmooth, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		if (graphId >= o_sectionGraphs.size())return;
	...
	ZSPACE_TOOLSETS_INLINE bool zTsNatpowerSDF::exportJSON(string pathCurrent, string dir, string filename, float printLyerWidth, float raftLayerWidth)
	{
		...
		for (int j = 0; j < end; j++)
		{
			...
			//for (int k = kStart; k < kEnd; k++)
			{
				...
				fnTrimGraph = zFnGraph(o_trimGraphs_cables[i]);
				...
				fnTrimGraph = zFnGraph(o_trimGraphs_interiorSplit[i]);
				//if (fnTrimGraph.numVertices() == 0) continue;
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_CableSectionPoints(int graphId, zObjGraph& o_cableGraph, zPointArray& outputPoints)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::combineMultipleGraphs(zObjGraphArray& inGraphs, zObjGraph& outGraph)
	{
		...
		for (zObjGraph& g : inGraphs)
		{
			...
			for (int i = 0; i < pts.size(); ++i)
			{
				...
				int tempIndex = -1;
				bool exist = coreUtils.checkRepeatVector(pts[i], positions, tempIndex);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::slotGraph_1(zPlane plane, zObjGraph& inPoly, float graphLength, bool iterate, zObjGraph& outGraph)
	{
		...
		for (zItGraphVertex v(inPoly); !v.end(); v++)
		{
			if (v.getColor() == _colorCornersStart)
			{
				startV = v.getPosition();
				counter++;
			}
			if (v.getColor() == _colorCornersEnd)
			{
				endV = v.getPosition();
				counter++;
			}
		...
		//for (zItGraphHalfEdge e(inPoly); !e.end(); e++)
		//{
		//	if (e.getStartVertex().getColor() == _colorCornersStart && e.getVertex().getColor() == _colorCornersEnd)
		//	{
		//		heEdge = e;
		//		startV = e.getStartVertex().getPosition();
		//		startEdgeId = e.getId();
		...
		//float partLength = iterate ? (edgeLength / 3.0) : (2.0 * edgeLength / 3.0);
		float partLength = iterate ? graphLength*1.5 : edgeLength - (graphLength*1.5);
		startV += (edgeVector * partLength);
		...
		//zPointArray gPts;
		//zIntArray gEdges;
		/*cout << endl << endl << "slot V " << startV << " | " << graphLength << endl << startV - (vector * (graphLength)) << endl << startV + (vector * (graphLength * 2.0));
		gPts.push_back(startV - (vector * (graphLength)));
		gPts.push_back(startV + (vector * (graphLength * 2.0)));
		gEdges.push_back(0);
		gEdges.push_back(1);
		...
		getPerpendicularVector(plane, edgeVector, startV, graphLength * 1.5, outGraph);
	...
	ZSPACE_TOOLSETS_INLINE int zTsNatpowerSDF::getGraphClosestPoint(zObjGraph& graph, zPoint& samplePoint, zPoint& outPoint, float& dist)
	{
		...
		for (zItGraphEdge e(graph); !e.end(); e++)
		{
			...
			e.getVertexPositions(pts);
			zPoint p;
			double d = coreUtils.minDist_Edge_Point(samplePoint, pts[0], pts[1],p);
			if (d < minDist)
			{
				minDist = d;
				index = e.getId();
	...
	ZSPACE_TOOLSETS_INLINE int zTsNatpowerSDF::getHeArrayClosestPoint(zItGraphHalfEdgeArray& hes, zPoint& samplePoint, zPoint& outPoint, float& dist)
	{
		...
		for (int i = 0; i < hes.size(); i++)
		{
			...
			double d = coreUtils.minDist_Edge_Point(samplePoint, p0,p1, tempCp);
			if (d < minDist)
			{
				minDist = d;
				index = i;
