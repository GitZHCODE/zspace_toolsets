...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_PrintBlocks(zDomainFloat& _printHeightDomain, float printLayerWidth, float raftLayerWidth, bool allSDFLayers, int& numSDFlayers, int funcNum, int numSmooth, zDomainFloat _neopreneOffset, bool compFrames, bool compSDF)
	{
		...
		for (int i = 1; i < o_sectionGraphs.size(); i++)
		{
			...
			compute_TrimGraphsHEs_BoundaryFeature(i, hes);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_PrintBlock_Sections(bool left, bool& outGeomChk)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			{
				outGeomChk = false;
				printf("\n outGeometryCheck FAILED for section [%i]!", i);
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_TrimGraphsHEs_Boundary_Arch(int graphId, zItGraphHalfEdgeArray& outHEs)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_TrimGraphsHEs_CableBracing(int graphId, zItGraphHalfEdgeArray& outHEs)
	{
		...
		/*for (int i = 0; i < gPositions.size(); i++)
		{
			for (int j = 0; j < gPositions.size(); j++)
			{
				if (i != j)
				{
					if (gPositions[i].distanceTo(gPositions[j]) < EPS)
					{
						printf("\n graph[%i] distance %1.4f", graphId, gPositions[i].distanceTo(gPositions[j]));
		...
		{
			int idprev = e.getId() + 1 % fnTrimG.numEdges();
			if (e.getLength() - (zItGraphEdge(o_trimGraphs[graphId], idprev)).getLength() < EPS)
			{
				printf("\n graph[%i] distance %1.4f", graphId, e.getLength());
				cout << endl <<
			}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_TrimGraphsHEs_BoundaryFeature(int graphId, zItGraphHalfEdgeArray& outHEs)
	{
				...
				else
				{
					...
					else
					{
						...
						if (blockType == zBlockType::Wall)
						{
							...
							compute_BlockSDF_Planar_wall(funcNum, numSmooth, i, (j % 2 == 0), printWidth, neopreneOffset, false, 0, raftWidth);

						}
						else
						{
							printf("\n SDF bracing");
							compute_BlockSDF_Planar_bracing(funcNum, numSmooth, i, (j % 2 == 0), printWidth, neopreneOffset, false, 0, raftWidth);
						}
					}
				}
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::compute_BlockSDF_Planar_bracing(int funcNum, int numSmooth, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		...
		for (zItGraphEdge e(o_sectionGraphs[graphId]); !e.end(); e++)
		{
			...
			e.getVertices(vs);
			if (vs[0].getColor() == _colorCornersInner && vs[1].getColor() == _colorCornersInner)
			{
	...
	ZSPACE_TOOLSETS_INLINE void zTsNatpowerSDF::slotGraph_1(zPlane plane, zObjGraph& inPoly, float graphLength, zObjGraph& outGraph)
	{
		...
		for (zItGraphEdge e(inPoly); !e.end(); e++)
		{
			zItGraphVertexArray vs;
			e.getVertices(vs);
			if (vs[0].getColor() == _colorCornersOuter && vs[1].getColor() == _colorCornersOuter)
			{
