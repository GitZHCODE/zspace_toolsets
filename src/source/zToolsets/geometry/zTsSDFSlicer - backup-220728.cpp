...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSDFSlicer::computePrintBlocks(float printPlaneSpacing, float printLayerWidth, float raftLayerWidth, bool allSDFLayers,  int &numSDFlayers, int funcNum, zDomainFloat neopreneOffset, bool compFrames, bool compSDF)
	{
		bool check = false;
		if (compFrames)
		{
			...
			//check = checkPrintLayerHeights(); 
		}

		if (compSDF)
		{
			computeSDF(allSDFLayers, numSDFlayers, funcNum,printLayerWidth, neopreneOffset.min, raftLayerWidth);
		}
	...
	ZSPACE_TOOLSETS_INLINE void zTsSDFSlicer::computeSDF(bool allSDFLayers, int& numSDFlayers,int funcNum, float printWidth, float neopreneOffset, float raftWidth)
	{
	...
	ZSPACE_TOOLSETS_INLINE bool zTsSDFSlicer::checkPrintLayerHeights()
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSDFSlicer::computeBlockSDF_Deck(int funcNum, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		...
		case 3:
			fnField.setFieldValues(slotField);
			break;
		...
		case 5:
			fnField.smoothField(booleanField_0, 1);
			fnField.setFieldValues(booleanField_0);
			break;
		}
	...
	ZSPACE_TOOLSETS_INLINE void zTsSDFSlicer::computeBlockSDF_Balustrade(int funcNum, int graphId, bool alternate, float printWidth, float neopreneOffset, bool addRaft, int raftId, float raftWidth)
	{
		if (graphId >= o_sectionGraphs.size())return;
		...
		if (funcNum >= 3) getScalars_3dp_trim(trimField, o_trimGraphs[graphId], 0.25 * pWidth, alternate);
		...
		case 5:
			fnField.smoothField(booleanField_1, 1);
	...
	ZSPACE_TOOLSETS_INLINE bool zTsSDFSlicer::exportJSON(string pathCurrent, string dir, string filename, float printLyerWidth, float raftLayerWidth)
	{
		...
		if (myfile.fail())
		{
			cout << "\n error in opening entryDir 0 " << blockMeshName.c_str() << endl;
			return false;
		}
		myfile << j.dump();
		myfile.close();
		...
		for (int j = 0; j < end; j++)
		{
			...
			for (int k = kStart; k < kEnd; k++)
			{
				...
				outName1 += "trim";
				outName1 += "_";
				outName1 += to_string(blockId) + "_" + format_number(j) + "_" + to_string(k) + ".json";
				...
				outName += filename;
				outName += "_";
				outName += to_string(blockId) + "_" + format_number(j) + "_" + to_string(k) + ".json";


				zFnGraph fnIsoGraph(o_contourGraphs[i]);
				zDoubleArray edgeLength;
				fnIsoGraph.getEdgeLengths(edgeLength);

				for (auto l : edgeLength)
				{
					if (l < distanceTolerance)
					{
						printf("\n edgeLength %1.7f", l);
					}
				}
				...
				zPoint* positions = fnIsoGraph.getRawVertexPositions();
				...
				for (zItGraphVertex v(o_contourGraphs[i]); !v.end(); v++)
				{
					if (!v.checkValency(2))
					{
						vArray.push_back(v);
						counterValence++;
						printf("\n %i %i %i", v.getId(), v.getHalfEdge().getId(), v.getHalfEdge().getVertex().getId());

					}
				}
				if (vArray.size() == 2)
				{
					zPoint p = vArray[1].getPosition();
					printf("\n %1.7f  ", vArray[0].getPosition().distanceTo(p));
					zItGraphHalfEdge he = vArray[0].getHalfEdge();
					vPositions.push_back(positions[vArray[0].getId()]);
					vSequence.push_back(vArray[0].getId());
					printf("\n ");
					do
					{
						//printf(" %i ", he.getVertex().getId());
						vPositions.push_back(positions[he.getVertex().getId()]);
						vSequence.push_back(he.getVertex().getId());

						eConnects.push_back(vPositions.size() - 2);
						eConnects.push_back(vPositions.size() - 1);

						he = he.getNext();
					} while (he.getVertex() != vArray[1]);
					vPositions.push_back(positions[vArray[1].getId()]);
				...
				if (vArray.size() == 0)
				{
					...
					vPositions.push_back(positions[he.getStartVertex().getId()]);
				...
				printf("\n edges - vertices - size() : %i - %i - %s - %i",fnIsoGraph.numEdges(), fnIsoGraph.numVertices(), (fnIsoGraph.numEdges() == fnIsoGraph.numVertices())? "" : "F", vArray.size()); 


				//fnIsoGraph.clear();
				zVector norm(sectionFrames[i](2, 0), sectionFrames[i](2, 1), sectionFrames[i](2, 2));

				//fnIsoGraph.create(vPositions, eConnects);
				printf("\n %i - %i - %s : %s ", vPositions.size(), eConnects.size(), (vPositions.size()== fnIsoGraph.numVertices())? "T": "vp-F", (eConnects.size() == fnIsoGraph.numHalfEdges()) ? "T" : "ne-F");

				fnIsoGraph.to(outName, zJSON);


				//printf("\n EXPORT JSON CHECK - 11-5 - %i : %i", j, k);
				...
				if (in_myfile.fail())
				{
					cout << "\n  error in opening entryDir 1 " << outName.c_str() << endl;
				}
				...
				for (zItGraphVertex v(o_contourGraphs[i]); !v.end(); v++)
				{
					...
					zPoint intPt;
					bool check = coreUtils.line_PlaneIntersection(p, p1, prevNorm, prevOrigin, intPt);
					//printf("\n EXPORT JSON CHECK - 11-9:2 - %i : %i : %i", j, k, i);
				...
				if (myfile.fail())
				{
					cout << "\n  error in opening entryDir 2 " << outName.c_str() << endl;
					return false;
				}
				//printf("\n EXPORT JSON CHECK - 11-12 - %i : %i", j, k);
				...
				//printf("\n EXPORT JSON CHECK - 11-14 - %i : %i", j, k);


			}
		}
		printf("\n counterValence %i ", counterValence);
	...
	ZSPACE_TOOLSETS_INLINE void zTsSDFSlicer::getScalars_3dp_trim(zScalarArray& scalars, zObjGraph& o_trimGraph, float offset, bool alternate)
	{
		...
		for (int i = 1; i < end - 1; i++)
		{
			...
			zPoint p0 = (alternate) ? midPt + e * offset : midPt - e * offset;
			zVector dir = upVec ^ e;
			dir.normalize();
...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE int EXT_checkLayerHeight(zTsSDFSlicer*& slicer, bool& check)
	{
		...
		check = slicer->checkPrintLayerHeights();
	}
	...
	ZSPACE_TOOLSETS_INLINE int EXT_computePrintBlocks(zTsSDFSlicer*& slicer, float printPlaneSpace, float printLayerWidth, float raftLayerWidth, bool allSDFLayers, int& numSDFLayers, int SDFFunc_Num, float neopreneOffsetMin, float neopreneOffsetMax, bool compFrames, bool compSDF)
	{
		...
		for ( printPlaneSpace = printHeightDomain.min; printPlaneSpace <= printHeightDomain.max; printPlaneSpace += 0.0005)
		{
			...
			bool frameCHECKS = slicer->checkPrintLayerHeights();
	...
	ZSPACE_TOOLSETS_INLINE void EXT_getPlanesData(vector<zTransform>* graph, float* matrix)
	{
		for (int i = 0; i < graph->size(); i++)
		{
			...
			/*matrix[i * 16 + 0] = frame(0, 0);
			matrix[i * 16 + 1] = frame(0, 1);
			matrix[i * 16 + 2] = frame(0, 2);
			matrix[i * 16 + 3] = frame(0, 3);
					   
			matrix[i * 16 + 4] = frame(1, 0);
			matrix[i * 16 + 5] = frame(1, 1);
			matrix[i * 16 + 6] = frame(1, 2);
			matrix[i * 16 + 7] = frame(1, 3);
					   
			matrix[i * 16 + 8] = frame(2, 0);
			matrix[i * 16 + 9] = frame(2, 1);
			matrix[i * 16 + 10] = frame(2, 2);
			matrix[i * 16 + 11] = frame(2, 3);
					   
			matrix[i * 16 + 12] = frame(3, 0);
			matrix[i * 16 + 13] = frame(3, 1);
			matrix[i * 16 + 14] = frame(3, 2);
			matrix[i * 16 + 15] = frame(3, 3);*/
	...
	ZSPACE_TOOLSETS_INLINE void EXT_getGraphsSetFromVector(zObjGraphArray* graphs, zObjGraph** outGraphArray)
	{
		for (int i = 0; i < graphs->size(); i++)
		{
			/*zFnGraph fn(graphs->at(i));
			outGraphArray[i] = fn.graphObj;*/
	...
	ZSPACE_TOOLSETS_INLINE void EXT_CheckFolder(char* folderDirectoryChar, float printPlaneSpace, float printLayerWidth, float raftLayerWidth, int SDFFunc_Num, float neopreneOffsetMin, float neopreneOffsetMax)
	{
		...
		for (auto const& entryDir : std::filesystem::directory_iterator{ folderDirectory })
		{
			if (entryDir.path().extension() == ".json")
			{
				...
				std::stringstream fileStream = stringstream(entryDir.path().filename().string());
				...
				if (nameSegments.size() == 2)
				{
					if (nameSegments[0] == "balustrade" || nameSegments[0] == "deck")
					{
						...
						for (printPlaneSpace = printHeightDomain.min; printPlaneSpace <= printHeightDomain.max; printPlaneSpace += 0.0005)
						{
							slicer.computePrintBlocks(printPlaneSpace, printLayerWidth, raftLayerWidth, true, sdfCount, SDFFunc_Num, neopreneOffset, true, false);
