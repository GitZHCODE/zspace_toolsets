...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE zSlimeAgent::zSlimeAgent()
	{
	...
	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::create(zVector &_pos, double &_SO, double &_SA, double &_RA, double &_depT, double &_pCD, double &_sMin)
	{
	...
	ZSPACE_TOOLSETS_INLINE zVector zSlimeAgent::getF()
	{
	...
	ZSPACE_TOOLSETS_INLINE zVector zSlimeAgent::getFR()
	{
		zVector axis(0, 0, 1);
	...
	ZSPACE_TOOLSETS_INLINE zVector zSlimeAgent::getFL()
	{
		zVector axis(0, 0, 1);
	...
	ZSPACE_TOOLSETS_INLINE double zSlimeAgent::getSO()
	{
		return *SO;
	}

	ZSPACE_TOOLSETS_INLINE double zSlimeAgent::getSA()
	{
		return *SA;
	}

	ZSPACE_TOOLSETS_INLINE double zSlimeAgent::getRA()
	{
		return *RA;
	}

	ZSPACE_TOOLSETS_INLINE double zSlimeAgent::getSMin()
	{
		return *sMin;
	}

	ZSPACE_TOOLSETS_INLINE double zSlimeAgent::getDepT()
	{
		return *depT;
	}
	...
	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setVelocity(double a_F, double a_FR, double a_FL, bool chemoRepulsive)
	{
		zVector out;
		zVector axis(0, 0, 1);
	...
	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setSO(double &_SO)
	{
		SO = &_SO;
	}

	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setSA(double &_SA)
	{
		SA = &_SA;
	}

	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setRA(double &_RA)
	{
		RA = &_RA;
	}

	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setDepT(double &_depT)
	{
		depT = &_depT;
	}

	ZSPACE_TOOLSETS_INLINE void zSlimeAgent::setSMin(double &_sMin)
	{
		sMin = &_sMin;
	}
...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE zSlimeEnvironment::zSlimeEnvironment()
	{

	}

	ZSPACE_TOOLSETS_INLINE zSlimeEnvironment::zSlimeEnvironment(zObjMeshField<zScalar> &_fieldObj)
	{
		fieldObj = &_fieldObj;
	...
	ZSPACE_TOOLSETS_INLINE double zSlimeEnvironment::getChemAatPosition(zVector &pos)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zSlimeEnvironment::diffuseEnvironment(double decayT, double diffuseDamp, zDiffusionType diffType)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zSlimeEnvironment::minMax_chemA(double min, double max)
	{
		minA = 10000;
		maxA = -10000;
...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_INLINE zTsSlime::zTsSlime()
	{
	...
	ZSPACE_TOOLSETS_INLINE zTsSlime::zTsSlime(zObjMeshField<zScalar> &_fieldObj, zObjPointCloud &_pointsObj)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::createEnvironment(zVector _minBB, zVector _maxBB, int _resX, int _resY, int _NR)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::createEnvironment(double _pix, int _res, zVector _minBB, int _NR)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::createAgents(double _p, double &_SO, double &_SA, double &_RA, double &_depT, double &_pCD, double &_sMin)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::slime_Motor(double dT, zIntergrationType integrateType, bool agentTrail)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::slime_Sensor(bool chemoRepulsive)
	{
		for (int i = 0; i < agents.size(); i++)
		{
			if (!agents[i].fnParticle.getFixed())
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::containBounds(int  index, double dT, zIntergrationType integrateType)
	{
	...
	ZSPACE_TOOLSETS_INLINE bool  zTsSlime::addFoodSource(int id, double depA, int nR, bool attractant)
	{
		bool out = false;
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::makeBoundaryRepellant(double depA, int distFromBoundary)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::depositChemicalA(int id, double depA)
	{
		environment.chemA[id] += depA;
	}

	ZSPACE_TOOLSETS_INLINE void zTsSlime::depositAtFoodSource(double depA, double wProj)
	{
	...
	ZSPACE_TOOLSETS_INLINE int zTsSlime::randomUnoccupiedCell(int boundaryOffset)
	{
		bool exit = false;
		int outId = -1;
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::clearFoodSources(bool Attractants, bool Repellants)
	{
		if (Attractants)
		{
			for (int i = 0; i < attractants.size(); i++)
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::randomRemoveAgents(int minAgents)
	{
	...
	ZSPACE_TOOLSETS_INLINE void zTsSlime::computeEnvironmentColor(bool dispAgents, bool usePercentile)
	{
		environment.setFieldValues(environment.chemA);
	}

	ZSPACE_TOOLSETS_INLINE void zTsSlime::updateAgentParametersFromField(int agentId, vector<double> &fieldScalars, double &parameterValue, double minVal, double maxVal, zSlimeParameter parm)
	{
