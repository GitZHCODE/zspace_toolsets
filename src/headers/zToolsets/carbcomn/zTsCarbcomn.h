...
// Author : Wo Lin <wo.lin@zaha-hadid.com>
//

#ifndef ZSPACE_TS_Carbcomn_SDF_H
...
namespace zSpace
{
	...
	{
	public:
		const float printWidthInterior = 0.042;
	...
	public:
		...
		const float offset_1st_interior = printWidthInterior / 2.0; //total 48mm layer width //24
		const float offset_1st_exterior = printWidthExterior / 2.0; //total 32mm layer width
		...
		const float bracingEdgeWidth = targetExteriorGap / 2.0f;
		const float bracingEdgeSlotWidth = (printWidthExterior - 0.012f) *0.5f; // 18mm gap
		...
		//cable
		const float cableWidth = 0.10;
		//iterating offset for slots
		const float slotIterating = 0.06;
		const float slotIterating_in = 0.04;
		...
		const float splitTrimOffset = offset_1st_exterior + (offset_2nd_exterior); // 28
		const float splitTrimTimming = offset_1st_interior + offset_2nd_interior;  // 44
	...
	*/
	...
	class ZSPACE_TOOLSETS zTsCarbcomn
	{
		//protected:
	public:
		//--------------------------
		...
		zObjGraphArray o_trimGraphs_bracing_slots_flat;
		zObjGraphArray o_trimGraphs_cableprofile;
...
#else
#include<source/zToolsets/carbcomn/zTsCarbcomn.cpp>
#endif
