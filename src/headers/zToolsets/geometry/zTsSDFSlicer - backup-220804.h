...
namespace zSpace
{
	...
	*/
	...
	public:
		...
		*	\since version 0.0.4
		*/
		bool checkPrintLayerHeights(zDomainFloat& layersHeight);
...
namespace zSpace
{
	...
	ZSPACE_TOOLSETS_EXT{
		...
		ZSPACE_TOOLSETS int EXT_computePrintBlocks(zTsSDFSlicer*& slicer, float printPlaneSpacing, float printLayerWidth, float raftLayerWidth, bool allSDFLayer, int& computeGraphCount, int funcNum,int SDFFunc_NumSmooth,  float neopreneOffsetMin, float neopreneOffsetMax, bool compFrames, bool compSDF);
		...
		//Iterate
		ZSPACE_TOOLSETS void EXT_CheckFolder(char* folderDirectoryChar, float printPlaneSpace, float printLayerWidth, float raftLayerWidth, int SDFFunc_Num, int SDFFunc_NumSmooth, float neopreneOffsetMin, float neopreneOffsetMax);
